...
@testset "RamboOnDiet" begin
    ...
    @testset "Massless constant weights" begin
        ...
        for n = 2:6
            if n == 3
                ...
                weights = [phase_space_weight(generate_point(rng, generator)) for _ = 1:64]
    ...
    @testset "Two-body isotropy in the center-of-mass frame" begin
        ...
        for _ = 1:3_000
            ...
            push!(
                cosines,
                LorentzVectorBase.pz(point[1]) /
                LorentzVectorBase.spatial_magnitude(point[1]),
            )
        end
    ...
    @testset "Three-body Dalitz checks through the main sampler" begin
        ...
        massless_events = [generate_point(rng, massless_generator) for _ = 1:12_000]
        ...
        massive_events = [generate_point(rng, massive_generator) for _ = 1:12_000]
